...
public class D_Quartet_Swapping {
    ...
    static int inversionCount(long a[]) {
    ...
    static void solve(int t) {
    	...
    	List<Long> oddIndex = new ArrayList<>();
    	List<Long> evenIndex = new ArrayList<>();
    	...
    	for(int i = 0, even = 0, odd = 0; i < n; ++i) {
    		
    		int j = i + 1;
    		
    		if(j % 2 == 1) {
    			
    			b[i] = oddIndex.get(odd++) + 1;
    			
    		} else {
    			
    			b[i] = evenIndex.get(even++) + 1;
    			
    		}
    		
    	}
    	
    	if(inversionCount(a) != inversionCount(b)) {
