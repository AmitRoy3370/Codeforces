...
public class Educational_Codeforces_Round_176_Rated_for_Div_2 {
    ...
    static int timer = 0, testCases, f, n, x_, y_, k, w, h, a11, b1, g, m, m1, m2, r, q, a_, b_, c_, start, end;
            ...
            for (int j = 64; j >= 0; --j) {
                ...
                for (k = 64; k >= 0; --k) {
                    ...
                    if (j >= i) {
                        ...
                        A[j][k] = Math.min(A[j][k], A[j - i][k] + pow(2L, i * 1L));

                    }

                    if (k >= i) {

                        A[j][k] = Math.min(A[j][k], A[j][k - i] + pow(2L, i * 1L));
    ...
    static void C(int i, long suffix[], ArrayList1<Long> list) {

        for (; !equal(i * 1L, m * 1L); ++i) {

            long leftSummation = suffix[i];
            long rightSummation = suffix[m - i];
            long commonBetweenLeftAndRight = isSmaller(i + "", (m - i) + "") ? suffix[(m - i)] : suffix[i];

            sum = Long.parseLong(sum(sum + "", Long.valueOf(sub((leftSummation * rightSummation) + "", commonBetweenLeftAndRight + "")) + ""));
    ...
    static void C(int i, long suffix[], TreeMap<Long, Integer> map) {

        for (; !equal(i * 1L, 0L); --i) {

            suffix[i] = Long.parseLong(sum(map.getOrDefault(i * 1L, 0) + "", suffix[i + 1] + ""));

        }

    }

    static void C(int i, TreeMap<Long, Integer> map) {

        for (; !equal(i * 1L, n * 1L); ++i) {

            map.put(a[i], Integer.valueOf(sum(map.getOrDefault(a[i], 0) + "", "1")));
    ...
    static void B(int t) {
        ...
        if (equal(k * 1L, 1L)) {
            ...
            if (isSmaller(a[0] + "", a[n - 1] + "")) {

                sum = Long.parseLong(sum(a[n - 1] + "", sum + ""));
                a[n - 1] = Integer.MIN_VALUE;

            } else {

                sum = Long.parseLong(sum(a[0] + "", sum + ""));
            ...
            for (int i = 0; i <= k; ++i) {
                ...
                sum = Long.parseLong(sum(sum + "", a[i] + ""));
    ...
    static void A(int t) {
        ...
        if (mod(N + "", 2L) == 0) {
            ...
            sum = Long.parseLong(sum(N + "", K + ""));

            sum = Long.parseLong(sub(sum + "", "2"));

            sum /= Long.parseLong(sub(K + "", "1"));

        } else {

            sum = Long.parseLong(sub(N + "", K + ""));
            sum = Long.parseLong(sum(sum + "", K + ""));
            sum = Long.parseLong(sub(sum + "", "2"));
            sum /= Long.parseLong(sub(K + "", "1"));
