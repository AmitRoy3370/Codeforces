...
public class A_Array_Balancing {
    ...
    static void solve(int i, long a[], long b[]) {
        ...
        long firstPossible = Math.abs(a[i + 1] - a[i]) + Math.abs(b[i + 1] - b[i]);

        long secondPossible = Math.abs(a[i + 1] - b[i]) + Math.abs(b[i + 1] - a[i]);
        ...
        sum += Math.min(firstPossible, secondPossible);
