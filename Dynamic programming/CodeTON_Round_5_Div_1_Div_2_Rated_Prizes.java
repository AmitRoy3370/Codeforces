...
public class CodeTON_Round_5_Div_1_Div_2_Rated_Prizes {
    ...
    static void C(int i, int dp[], int present_occurance[], int diff[]) {
        ...
        if (present_occurance[(int) a[i]] != -1) {

            int j = present_occurance[(int) a[i]];

            dp[i + 1] = Math.max(dp[i + 1], dp[j] - j + i + 1);

        }

        int prev = diff[(int) a[i]];

        if (prev <= (dp[i] - i)) {

            present_occurance[(int) a[i]] = i;

            diff[(int) a[i]] = dp[i] - i;

        }

        C(i + 1, dp, present_occurance, diff);
    ...
    static void C(int t) {
        ...
        int present_occurance[] = new int[n + 1];

        int diff[] = new int[n + 1];

        for (int i = 0; i <= n; ++i) {

            present_occurance[i] = -1;
            diff[i] = -100 * n;

        }

        C(0, dp, present_occurance, diff);
    ...
    static long B(long xor, int i, long a[]) {
    ...
    static void A(int t) {
        ...
        long tsondo = sum(a, 0, 0);

        long tenzing = sum(b, 0, 0);

        if (tsondo > tenzing) {

            ans.append("Tsondu");

        } else if (tsondo < tenzing) {
