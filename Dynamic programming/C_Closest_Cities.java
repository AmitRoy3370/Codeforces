...
public class C_Closest_Cities {
    ...
    static void solve(int t) {
    	...
    	for(int i = 1; i != n - 1; ++i) {
    		...
    		left[i] = Math.abs(a[i] - a[i - 1]);
    		right[i] = Math.abs(a[i] - a[i + 1]);
    	...
    	for(int i = 1; i != n; ++i) {
    		...
    		if(dp[i] == 1) {
    			...
    			left[i] = left[i - 1] + Math.abs(a[i] - a[i - 1]);
    	...
    	for(int i = n - 2; i >= 0; --i) {
    		...
    		if(dp[i] == 1) {
    			...
    			right[i] = right[i + 1] + Math.abs(a[i] - a[i + 1]);
    	...
    	for(int i = 0; i != m; ++i) {
    		...
    		if(b[i] > c[i]) {
    			...
    			sb.append(Math.abs(left[start] - left[end])).append(" ");
    			
    		} else {
    			
    			sb.append(Math.abs(right[start] - right[end])).append(" ");
