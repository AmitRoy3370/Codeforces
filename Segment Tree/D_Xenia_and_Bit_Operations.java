...
public class D_Xenia_and_Bit_Operations {
    ...
    private static class SegmentTree {
    	...
    	public SegmentTree(long a[]) {
    		...
    		build(a, 1, 0, n - 1, height);
    		
    	}
    	
    	private void build(long a[], int node, int start, int end, int height) {
    		...
    		if(start == end) {
    			...
    			build(a, 2 * node, start, mid, height - 1);
    			build(a, 2 * node + 1, mid + 1, end, height - 1);
    			
    			tree[node] = (height % 2 == 1) ? (tree[2 * node] | tree[2 * node + 1]) : (tree[2 * node] ^ tree[2 * node + 1]);
    			
    		}
    		
    	}
    	
    	private void update(int node, int start, int end, int index, long value, int height) {
    		...
    		if(start == end) {
    			...
    			if(index <= mid) {
    				...
    				update(2 * node, start, mid, index, value, height - 1);
    				
    			} else {
    				
    				update(2 * node + 1, mid + 1, end, index, value, height - 1);
    	...
    	public void update(int index, long value) {
    		...
    		update(1, 0, n - 1, index, value, height);
