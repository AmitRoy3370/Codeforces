    ...
    static void solve(int t) {
    	...
    	for(int i = 0; i < n; ++i) {
    		...
    		while(X > 0) {
    			...
    			Element first = set.first();
    			
    			set.remove(first);
    			
    			set.add(new Element(first.first + 1, index++));
    			
    			--X;
    			
    		}
    		
    		Element last = set.last();
    		
    		set.remove(last);
    		
    		set.add(new Element(0, index++));
    		
    	}
    	
    	long maxVal = set.last().first;
        long totalValue = maxVal + k - a[n - 1];
    	
    	ans.append(totalValue);
    ...
    private static class Element implements Comparable<Element> {
        ...
        public int compareTo(Element e) {
              ...
              if (this.first != e.first) {
                return Long.compare(this.first, e.first);
            }
            return Long.compare(this.second, e.second);
