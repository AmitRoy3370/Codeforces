...
public class D_Inaccurate_Subsequence_Search {
    ...
    static void solve(int t) {
    	...
    	long currentMatches = 0L;
    	long good = 0L;
    	
    	for(long i : countB.keySet()) {
    		
    		if(countA.containsKey(i)) {
    			
    			currentMatches += Math.min(countB.get(i), countA.get(i));
    	...
    	if(n == m) {
    		...
    		for(int i = 0; i < n - m; ++i) {
    			...
    			currentMatches -= Math.min(countA.getOrDefault(a[i], 0), countB.getOrDefault(a[i], 0));
    			currentMatches -= Math.min(countA.getOrDefault(a[i + m], 0), countB.getOrDefault(a[i + m], 0));
    			
    			countA.put(a[i], countA.getOrDefault(a[i], 0) - 1);
    			countA.put(a[i + m], countA.getOrDefault(a[i + m], 0) + 1);
    			
    			currentMatches += Math.min(countA.get(a[i]), countB.getOrDefault(a[i], 0));
    			currentMatches += Math.min(countA.get(a[i + m]), countB.getOrDefault(a[i + m], 0));
