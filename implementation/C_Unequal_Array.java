...
public class C_Unequal_Array {
    ...
    static int solve(int index, long a[], int increase) {

        if (index + increase >= n || index + increase < 0) {

            return -1;

        }

        if (index + increase < n && index + increase >= 0 && a[index] == a[index + increase]) {

            return index;

        }

        return solve(index + increase, a, increase);
    ...
    static void solve(int t) {
        ...
        int len = Integer.parseInt(sum(sub(String.valueOf(end), String.valueOf(start)), String.valueOf(1)));
