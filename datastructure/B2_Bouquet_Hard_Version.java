...
public class B2_Bouquet_Hard_Version {
    ...
    static long A_, B_, N_, K, sum = 0L, G, a1, ak, X, Y, A, B, C, l_, t_, P, M;
    	...
    	for(long i : map.keySet()) {
    		...
    		if(l >= firstElement) {
    			...
    			if(map.containsKey(i + 1)) {
    				...
    				long secondElement = (i + 1) * map.get(i + 1);
    				...
    				if(l >= secondElement) {
    					...
    					long first = map.get(i + 1) - (p - l) / (i + 1);
    					long second = map.get(i);
    					
    					l -= Math.min(Math.min(first, second), l);
    			...
    			if(map.containsKey(i + 1)) {
    			  ...
    			  long first = map.get(i + 1);
    			  long second = (p - l) / i;
    					
    			   l -= Math.min(Math.min(second, l), first);
    			
    			}
    					
    		}
    		
    		sum = Math.max(sum, M - l);
