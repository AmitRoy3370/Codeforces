...
public class D_Shop_Game {
    ...
    static int testCases, n, x_, y_, k, count = 1, d, w, h, a11, b1, g, m, s, r, q, a_, b_, c_, N = (int) (1e6 + 1);
    ...
    static void solve(int t) {
    	...
    	List<Pair_> list = new ArrayList<>();
    	
    	for(int i = 0; i < n; ++i) {
    		
    		list.add(new Pair_(b[i], a[i]));
    	...
    	for(int i = n - 1; i >= 0; --i) {
    		...
    		sum -= Math.max(f[i][0] - f[i][1], 0L);
    		
    		pq.add(new Element(f[i][1]));
    		
    		while(!pq.isEmpty() && pq.size() > k) {
    			
    			discount -= pq.peek().first;
    		...
    		if(pq.size() == k) {
    			...
    			totalEarn = Math.max(totalEarn, sum - discount);
    			
    		}
    		
    	}
    	
    	ans.append(totalEarn);
