...
public class Educational_Codeforces_Round_184_Rated_for_Div_2 {
    ...
    static int timer = 0, testCases, f, n, x_, y_, k, w, h, a11, b1, g, m, m1, m2, r, q, a_, b_, c_, start, end, l, _p;

    static char x[], y[], z[], _A[][], _B[][];

    static long a[], b[], c[], suffix[], prefix[], A[][], B[][], d[], dp[];

    static double time[], quries[];

    static int p[], trie[], u[], v[], u1[], v1[];

    static String s1[];

    static long seive[], factorial[], L[], R[], inverseFactorial[];

    static long N = (long) (1e5 + 10), K, M, sum = 0L, sub, mul, G, a1, ak, X, Y, C, l_, t_, P, R_, N_, X1, Y1, Z;

    static long xs, ys, xt, yt;

    static char mat[][];

    static long H, mod = (long) (998244353);
    ...
    static long D1(long mid, int n, long Y) {
    ...
    static void D1(int t) {
        ...
        long value = (long)(1e12);
        
        if(D1(value, (int)X, Y) < K) {
            
            ans.append("-1");
            
        } else {
            
            long left = 1, right = value;
    ...
    static void C(int t) {
        ...
        long base = prefix[n - 1];

        long minF = 0L, best = 0L;

        for (int i = 1; i <= n; ++i) {

            long fr = pow(i * 1L, 2L) + i - prefix[i - 1];

            best = Math.max(best, fr - minF);

            minF = Math.min(minF, fr);

        }

        base += Math.max(0L, best);
    ...
    static void A(int t) {
        ...
        int lowerBound = lower_bound(a, K + 1);
        int upperBound = upper_bound(a, K - 1);
    ...
    public static void main(String[] Tania) throws IOException {
    ...
    private static void E1_input() throws IOException {
    ...
    static int upper_bound(long[] arr, long value) {
        ...
        while (low < high) {
            ...
            if (arr[mid] <= value) {
                ...
                high = mid; // Narrow down to the left
            }
        }
        return low; // Index of the first element greater than value
    }
    ...
    static long max_element(long a[], int i, int n, long max) {
        ...
        return max_element(a, i + 1, n, max);

    }

    static long min_element(long a[], int i, int n, long min) {
        ...
        return min_element(a, i + 1, n, min);
