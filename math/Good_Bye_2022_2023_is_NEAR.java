...
public class Good_Bye_2022_2023_is_NEAR {
    ...
    static void initialize(int i, Map<Long, Integer> map) {
    ...
    static void build(int i, Map<Long, Integer> map, int k) {
        ...
        map.put((long) (a[i] % k), map.getOrDefault((long) (a[i] % k), 0) + 1);

        build(i + 1, map, k);

    }

    static boolean solveC(int k) {
        ...
        for (int i = 0; i < k; ++i) {
            ...
            if (map.getOrDefault((long) i, 0) <= 1) {
