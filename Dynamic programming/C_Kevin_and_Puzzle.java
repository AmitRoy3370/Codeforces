...
public class C_Kevin_and_Puzzle {
    ...
    static void solve(int t) {
    	...
    	TreeMap<Long, Integer> honest = new TreeMap<>();
    	TreeMap<Long, Integer> liar = new TreeMap<>();
    	
    	honest.put(0L, 1);
    	
    	for(int i = 0; i < n; ++i) {
    		
    		sum = (honest.getOrDefault(a[i], 0) + liar.getOrDefault(a[i], 0)) % mod;
    		
    		TreeMap<Long, Integer> presentHonest = new TreeMap<>();
    		TreeMap<Long, Integer> presentLiar = new TreeMap<>();
    		
    		if(sum > 0) {
    			
    			presentHonest.put(a[i], (int)sum);
    			
    		}
    		
    		for(long lie : honest.keySet()) {
    			
    			presentLiar.put(lie + 1, presentLiar.getOrDefault(lie + 1, 0) + honest.get(lie));
    			
    		}
    		
    		honest = presentHonest;
    		liar = presentLiar;
