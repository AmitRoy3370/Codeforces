...
public class D_Tandem_Repeats {
    ...
    static void solve(int t) {
    	...
    	for(int firstHalfLen = 1; firstHalfLen != n / 2 + 1; ++firstHalfLen) {
    		
    		for(int j = 1; j != n + 1; ++j) {
    			
    			if(j + firstHalfLen - 1 >= n) {
    				
    				continue;
    				
    			}
    			
    			if(x[j - 1] == x[firstHalfLen + j - 1] || x[j - 1] == '?' || x[j + firstHalfLen - 1] == '?') { 
    				
    				dp[firstHalfLen][j] = dp[firstHalfLen][j - 1] + 1L;
    				
    			}
    			
    			if(dp[firstHalfLen][j] == firstHalfLen) {
    				
    				maxHalfLen = Math.max(maxHalfLen, firstHalfLen);
    	...
    	int fullLength = maxHalfLen * 2;
    	
    	ans.append(fullLength);
