...
public class F_Eating_Candies {
    ...
    private static int n, testCases;
    ...
    private static void solve(int t) {
		...
		long sum_front = 0, sum_back = 0;
		
		long finalAns = Integer.MIN_VALUE;
		
		int index = 0, index_front = 0, index_back = n - 1;
		...
		while(size > 0) {
			...
			if(sum_front <= sum_back) {
				...
				sum_front += a[index_front++];
				--size;
				
			} else {
				
				sum_back += a[index_back--];
				--size;
			...
			if(sum_back == sum_front ) {
				...
				finalAns = Math.max(finalAns, n - size );
				
			}
			
		}
		
		ans.append((finalAns == Integer.MIN_VALUE ? 0 : finalAns) );
