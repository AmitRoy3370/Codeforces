...
public class E_Permutation_of_Rows_and_Columns {
    ...
    static int p[][], trie[], A[][], B[][];

    static String s1[];

    static long seive[], factorial[];

    static long A_, B_, N_, K, sum = 0L, G, a1, ak, X, Y, C, l_, t_, P;
    ...
    static void solve(int t) {
    	...
    	for(int j = 0; j < m; ++j) {
    		...
    		int position = col[B[0][j]];
    		
    		for(int i = 1; i < n; ++i) {
    			
    			if(col[B[i][j]] != position) {
    	...
    	if(!yes) {
    		...
    		for(int i = 0; i < n; ++i) {
    			...
    			int position = raw[B[i][0]];
    			
    			for(int j = 1; j < m; ++j) {
    				
    				if(raw[B[i][j]] != position) {
