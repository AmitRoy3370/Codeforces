...
public class D_Not_Alone {
    ...
    static long calculate(int len) {
    	...
    	for(int i = 1; i <= n; ++i) {
    		...
    		if(i >= 2) {
    			...
    			dp[i] = Math.min(dp[i], dp[i - 2] + Math.abs(a[i + len - 1] - a[i + len - 2]));
    			
    		}
    		
    		if(i >= 3) {
    			
    			dp[i] = Math.min(dp[i], dp[i - 3] + Math.max(a[i + len - 1], Math.max(a[i + len - 2], a[i + len - 3])) - Math.min(a[i + len - 1], Math.min(a[i + len - 2], a[i + len - 3])));
    ...
    static void solve(int t) {
    	...
    	ans.append(Math.min(calculate(1), Math.min(calculate(2), calculate(3))));
