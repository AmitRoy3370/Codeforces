...
public class Codeforces_Round_975_Div_2 {
    ...
    static long N = 10000010, K, M, sum = 0L, G, a1, ak, X, Y, C, l_, t_, P, R_, N_, X1, Y1;
    ...
    static long D(int i, int T, int lastPosition, int nextPosition, int logN) {
        ...
        if (lastPosition == 0 && nextPosition == n - 1) {

            return 1;

        }

        return D(lastPosition - 1, T + 1, lastPosition - 1, nextPosition, logN)
                + D(nextPosition + 1, T + 1, lastPosition, nextPosition + 1, logN);

    }

    static void D(int t) {

        m = 10 + (int) (Math.log(n) / Math.log(2));
    ...
    static void B(int t) {
        ...
        for (int i = 1; i <= n; ++i) {
            ...
            long point = (long)((long)(i) * (long)(n - i + 1)) - 1;

            map.put(point, map.getOrDefault(point, 0L) + 1L);

            if (i < n) {

                long difference = a[i] - a[i - 1] - 1;

                if (difference > 0) {

                    map.put((long) ((long)(i) * (long)(n - i)), map.getOrDefault((long) ((long)(i) * (long)(n - i)), 0L) + difference);
    ...
    static long A(int i, long cost, long max, int count) {

        if (i >= n) {

            return cost = max + count;

        }

        max = Math.max(a[i], max);

        return A(i + 2, cost, max, count + 1);

    }

    static void A(int t) {

        ans.append(Math.max(A(0, 0L, Long.MIN_VALUE, 0), A(1, 0L, Long.MIN_VALUE, 0)));
