...
public class C_Rooks_Defenders {
	
	static int testCases, n, types;
	
    static Scanner in = new Scanner();

    static PrintWriter out = new PrintWriter(System.out);

	static Map<Integer, Integer> raw;
	static Map<Integer, Integer> col;
	static TreeSet<Integer> rawLeft;
	static TreeSet<Integer> colLeft;
	...
	static void solve() throws IOException {
		...
		if(types == 1) {
			...
			if(rawLeft.contains(x) ) {
				
				rawLeft.remove(x);
				
			}
			
			if(colLeft.contains(y) ) {
			...
			if((rawLeft.floor(x1) < x1 && rawLeft.ceiling(x1) > x2) || (colLeft.floor(y1) < y1 && colLeft.ceiling(y1) > y2) ) {
