...
public class Codeforces_Round_991_Div_3 {
    ...
    static void G(int t) {
        ...
        for (int i = 0; i < n - 1; ++i) {
            ...
            maxEdge = Math.max(sum, maxEdge);

        }

        ans.append(Math.max(maxEdge, maxDegree));
    ...
    static void F(int t) {
        ...
        for (int i = 1, j = 0; i < n; ++i, ++j) {
            ...
            b[j] = Math.abs(a[i] - a[i - 1]);

        }

        SegmentTree segment = new SegmentTree(b);

        StringBuilder sb = new StringBuilder();

        for (int i = 0; i < m - 1; ++i) {

            sb.append(segment.rangeGCD(u[i], v[i] - 1)).append(" ");

        }

        sb.append(segment.rangeGCD(u[m - 1], v[m - 1] - 1));
