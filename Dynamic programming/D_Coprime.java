...
public class D_Coprime {
    ...
    static int testCases, n, m, count = 0;
    ...
    static f solve(int left, int right, int count) {

        if (left >= right) {

            return new f(new long[]{a[left], a[right]}, count);

        }

        int mid = (left + right) / 2;

        f first_partion = solve(left, mid, count);
        f second_partion = solve(mid + 1, right, count);

        long first[] = first_partion.a;
        long second[] = second_partion.a;
        ...
        first_partion.a = first;
        second_partion.a = second;

        if (first[1] == second[0] - 1 && second[1] - first[0] == (right - left)) {

            return new f(new long[]{first[0], second[1]}, count);
    ...
    static void solve(int t) {
        ...
        for (long i : a) {
            ...
            dp[(int) i] = Math.max(dp[(int) i], index);
        ...
        for (int i = 1; i <= 1000; ++i) {
            ...
            for (int j = 1; j <= 1000; ++j) {
                ...
                if (gcd((long) i, (long) j) == 1 && dp[i] >= 1 && dp[j] >= 1) {

                    final_ans = Math.max(final_ans, dp[i] + dp[j]);
