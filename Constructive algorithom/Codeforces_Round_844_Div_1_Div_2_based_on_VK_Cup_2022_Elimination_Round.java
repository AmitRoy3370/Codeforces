...
public class Codeforces_Round_844_Div_1_Div_2_based_on_VK_Cup_2022_Elimination_Round {
    ...
    static int testCases, n, k, count = 0, d, w, h, a11, b1, f, g;
    ...
    static void C(int t) {
        ...
        element e[] = new element[26];

        List<element> list = new ArrayList<>();

        for (int i = 0; i < 26; ++i) {

            e[i] = new element(count[i], i);
		...
		for(int i = 1; i <= 26; ++i) {
			...
			for(int j = 0; j < i; ++j) {
				...
				dif += Math.min(per_element, e[j].element);
		...
		for(int i = 0; i < ans_i; ++i) {
			...
			count2[e[i].index] = n / ans_i;
			
		}
		
		char final_ans[] = new char[n];
		...
		for(int i = 0; i < n; ++i) {
			...
			while(j < count2.length && count2[j] == 0) {
		...
		ans.append((n - same)).append("\n").append(new String(final_ans));
    ...
    static boolean valid(int i) {
        ...
        return a[i] < i + 1 && (String.valueOf(a[i + 1]).equalsIgnoreCase(String.valueOf(i + 2)) || a[i + 1] > i + 2);

    }

    static int solveB(int i, int final_ans) {
    ...
    static void B(int t) {
        ...
        int final_ans = (a[0] != 0 ? 2 : 1);

        final_ans = solveB(0, final_ans);
    ...
    static void A(int t) {
        ...
        int a3 = (int) (f1) + w - a11 + w - f;

        int a4 = (int) Math.abs(f1) + a11 + f;

        int first_one = Math.min(a1, a2);

        int second_one = Math.min(a3, a4);

        final_ans = Math.min(first_one, second_one);

        final_ans = Integer.parseInt(sum(final_ans + "", String.valueOf(h)));
