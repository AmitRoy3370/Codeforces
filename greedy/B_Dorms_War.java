...
public class B_Dorms_War {
	...
	static char password[], spetial[];
	
	static StringBuilder answer = new StringBuilder();
	
	static void solve() {
		
		boolean spetial_Character_present[] = new boolean[26];
		
		int ans = 0;
		
		for(char i : spetial) {
			
			spetial_Character_present[i - 'a'] = true;
			
		}
		
		int last_spetial_character = 0;
		
		for(int i = 0; i < n; ++i) {
			
			if(spetial_Character_present[password[i] - 'a']) {
				
				ans = Math.max(ans, i - last_spetial_character);
				
				last_spetial_character = i;
