...
public class Codeforces_Round_853_Div_2_ {
    ...
    static int testCases, n, k, count = 0, d, w, h, a11, b1, f, g, m, s, r;

    static char x[], y[];

    static long a[], b[], c[], p[], v[];
    ...
    static void C(int t) {
        ...
        long counting[] = new long[n + m + 1];

        for (long i : a) {

            counting[(int) i] = (long)((long)m + 1L);
        ...
        for (int i = 0; i < m; ++i) {
            ...
            counting[(int)a[(int) p[i]]] -= (long)((long)m - (long)i);
            counting[(int) v[i]] += (long)((long)m - (long)i);

            a[(int) p[i]] = v[i];

        }

        long final_ans = (long)((long)n * (long)m * (long)((long)m + 1L));

        for (int i = 0; i <= n + m; ++i) {

            final_ans -= (counting[i] * (counting[i] - 1L)) / 2L;
    ...
    static void B(int t) {
        ...
        boolean equal_pair = true;

        int i = 0;

        for (i = 0; i <= n / 2; ++i) {

            char last = list.get(0);
    ...
    static void A(int t) {
        ...
        long gcd = 1L, min = Integer.MAX_VALUE;

        for (int i = 0; i < n; ++i) {

            for (int j = i + 1; j < n; ++j) {

                min = Math.min(min, gcd(a[i], a[j]));
