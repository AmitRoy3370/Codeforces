...
public class C_Messenger_in_MAC {
    ...
    static void dp(int t) {
    	...
    	Message message[] = new Message[n + 1];
    	
    	message[0] = new Message();
    	
    	for(int i = 1; i != n + 1; ++i) {
    		
    		message[i] = new Message(a[i], b[i]);
    		
    	}
    	
    	Arrays.sort(message);
    	
    	long twoDp[][] = new long[n + 1][n + 1];
    	
    	for(int i = 0; i != n + 1; ++i) {
    		
    		for(int j = 0; j != n + 1; ++j) {
    			
    			twoDp[i][j] = Long.MAX_VALUE;
    	...
    	for(int start = 1; start != n + 1; ++start) {
    		...
    		for(int end = start; end != n + 1; ++end) {
    			...
    			if(start == 1) {
    				...
    				twoDp[start][end] = message[end].A;
    				
    			} else {
    				
    				twoDp[start][end] = Math.min(twoDp[start][end], dp[end - 1] + message[end].A + message[end].B);
    		...
    		for(int end = start; end != n + 1; ++end) {
    			...
    			if(start == end) {
    				...
    				dp[end] = twoDp[start][end] - message[end].B;
    				
    			} else {
    				
    				dp[end] = Math.min(dp[end - 1], twoDp[start][end] - message[end].B);
    	...
    	ans.append(readMessage == -1 ? 0 : readMessage);
    ...
    static void solve(int t) {
    	...
    	for(int i = 1; i <= n; ++i) {
    		...
    		for(int j = i; j <= n; ++j) {
    			...
    			pq.add(new pair(message[j].A));
    			
    			while(!pq.isEmpty() && sum + message[j].B - message[i].B > N_) { 
    			...
    			messageSetLength = Math.max(messageSetLength, pq.size());
    			
    		}
    		
    	}
    	
    	ans.append(messageSetLength);
