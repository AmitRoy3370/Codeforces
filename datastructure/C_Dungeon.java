...
public class C_Dungeon {
    ...
    static void solve(int t) {
    	...
    	for(int i = 0; i < m; ++i) {
    		...
    		Map.Entry<Long, Integer> entry = map.ceilingEntry(element[i].first);
    		...
    		map.put(Math.max(element[i].second, sword), map.getOrDefault(Math.max(element[i].second, sword), 0) + 1);
    ...
    private static class Element implements Comparable<Element> {
        ...
        public int compareTo(Element other) {
            ...
            if ((this.second == 0) ^ (other.second == 0)) {
                return Long.compare(other.second, this.second); // c==1 first
            }
            return Long.compare(this.first, other.first); // then smaller b first
        }
