// Problem: D1. DFS Checker (Easy Version)
...
public class D1_DFS_Checker_Easy_Version {
    ...
    static void create(int i, int x, List<TreeSet<Integer>> list[], int p[]) {
    	...
    	list[p[i]].get(0).add(p[i + 1]);
    	list[p[i]].get(0).add(p[i + x]);
    	
    	create(i + 1, x / 2, list, p);
    	create(i + x, x / 2, list, p);
    	
    }
    
    static void swap(int x, int y, int diff, List<TreeSet<Integer>> list[], int parent[], int current[]) {
    	...
    	list[px].get(0).remove(x);
    	list[py].get(0).remove(y);
    	
    	list[px].get(0).add(y);
    	list[py].get(0).add(x);
    	
    	TreeSet<Integer> adjX = new TreeSet<>();
    	TreeSet<Integer> adjY = new TreeSet<>();
    	
    	for(int i : list[x].get(0)) {
    		
    		adjX.add(i);
    		
    	}
    	
    	for(int i : list[y].get(0)) {
    ...
    static void solve(int t) {
    	...
    	for(int i = 1; i <= n; ++i) {
    		...
    		TreeSet<Integer> set = list[i].get(0);
    	...
    	for(int j = 1; j <= q; ++j) {
    		...
    		swap(p[m], p[k], different, list, parent, current);
