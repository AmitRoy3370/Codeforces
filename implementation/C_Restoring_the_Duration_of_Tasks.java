...
public class C_Restoring_the_Duration_of_Tasks {
    ...
    static void solve(int index, boolean ok, long max, ArrayList1<Long> list) {
        ...
        max = Math.max(max, b[index - 1]);

        list.add(Math.max(0, b[index] - max));

        solve(index + 1, ok, max, list);
    ...
    static void solve(int t) {
        ...
        list.add(Long.parseLong(sub(b[0] + "", a[0] + "")));

        long max = b[0];

        solve(1, true, max, list);

        while (!list.isEmpty()) {

            ans.append(list.get(0)).append(" ");
