...
public class B_Getting_Zero {
    ...
    static void solve() {
        ...
        ArrayList<Long> dp[] = new ArrayList[(int) mod + 8];
        ...
        for (int i = 0; i < len; ++i) {
            ...
            dp[(i + 1) % len].add((long) i);

            dp[(i * 2) % len].add((long) i);
        ...
        while (!queue.isEmpty()) {
            ...
            while (dp[(int) x].head != null) {

                long data = dp[(int) x].head.data;

                if (a[(int) data] == -1) {

                    a[(int) data] = a[(int) x] + 1;

                    queue.add(data);

                }

                dp[(int) x].head = dp[(int) x].head.next;
