...
public class D_Token_Removing {
    ...
    static void solve(int t) {
    	...
    	int tokenAlreadyTaken = 0;
    	
    	for(int step = n; step >= 1; --step) {
    		
    		for(int token = tokenAlreadyTaken; token >= 0; --token) {
    			...
    			long way = (dp[token] * remain) % mod;
    			way = (way * step) % mod;
    			
    			dp[token + 1] = (dp[token + 1] + way) % mod;
    			
    		}
    		
    		if(dp[tokenAlreadyTaken + 1] >= 1) {
