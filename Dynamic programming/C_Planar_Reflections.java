...
public class C_Planar_Reflections extends Thread {
    ...
    static long solve(int current, int k, int direction) {
        ...
        if (dp[current][k][direction] != -1) {

            return dp[current][k][direction];
        ...
        if (direction == 1) {
            ...
            if (current < n) {
                ans += solve(current + 1, k, direction) - 1;
            }

            ans %= mod;

            if (current > 1) {
                ans += solve(current - 1, k - 1, 1 - direction) - 1;
            }

            ans %= mod;
            dp[current][k][direction] = ans;
    ...
    public static void main(String[] amit) throws IOException {
        ...
        new Thread(null, new C_Planar_Reflections(), "Main", 1 << 28).start();
    ...
    public void run() {
        ...
        try {
            ...
            //   Logger.getLogger(C_Planar_Reflections.class.getName()).log(Level.SEVERE, null, ex);
        }
