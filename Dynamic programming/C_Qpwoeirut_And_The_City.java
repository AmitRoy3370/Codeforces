...
public class C_Qpwoeirut_And_The_City {
    ...
    static long solve(int i, int end, long finalAns, int diff, long prefix[], int j) {
        ...
        //long adder = (Math.max(a[i - 1], a[i + 1]) + 1L - a[i]);
        finalAns = Math.max(Math.max(a[i - 1], a[i + 1]) + 1L - a[i], 0L);

        prefix[j] = prefix[j - 1] + finalAns;

        return solve(i + diff, end, finalAns, diff, prefix, j + 1);
    ...
    static void solve(int t) {
        ...
        long prefix1[] = new long[n / 2 + 1];
        long prefix2[] = new long[n / 2 + 1];
        ...
        if (n % 2 >= 1) {
            ...
            for (int i = 1; i < n - 1; i += 2) {
                ...
                finalAns += Math.max(Math.max(a[i - 1], a[i + 1]) + 1 - a[i], 0L);
            ...
            ans1 = solve(1, n - 1, ans1, 2, prefix1, 1);

            long ans2 = 0;

            ans2 = solve(2, n - 1, ans2, 2, prefix2, 1);

            //long ans3 = Long.MAX_VALUE;
            for (int i = 0; i < n / 2; ++i) {

                finalAns = Math.min(finalAns, prefix1[i] + prefix2[n / 2 - 1] - prefix2[i]);

            }

            //finalAns = Math.min(ans1, Math.min(ans2, ans3));
        }
