...
public class D_Scammy_Game_Ad {
    ...
    static class Gate {
    	...
    	public Gate(boolean isMul, long value) {
    ...
    static void solve(int t) {
    	...
    	Gate gateLeft[] = new Gate[n];
    	Gate gateRight[] = new Gate[n];
    	
    	for(int i = 0; i < n; ++i) {
    		
    		if(x[i] == '+') {
    			
    			gateLeft[i] = new Gate(false, L[i]);
    			
    		} else {
    			
    			gateLeft[i] = new Gate(true, L[i]);
    			
    		}
    		
    		if(y[i] == '+') {
    			
    			gateRight[i] = new Gate(false, R[i]);
    			
    		} else {
    			
    			gateRight[i] = new Gate(true, R[i]);
    			
    		}
    		
    	}
    	
    	long leftLaneValue = 1L;
    	long rightLaneValue = 1L;
    	long additionalValue = 0L;
    	
    	for(int i = n - 1; i >= 0; --i) {
    		
    		long maxValue = Math.max(leftLaneValue, rightLaneValue);
    		
    		long multiplicationLeft = 0;
    		long additionLeft = 0;
    		
    		if(gateLeft[i].isMul) {
    			
    			multiplicationLeft = gateLeft[i].value - 1;
    			
    		} else {
    			
    			additionLeft = gateLeft[i].value;
    			
    		}
    		
    		long multiplicationRight = 0;
    		long additionRight = 0;
    		
    		if(gateRight[i].isMul) {
    			
    			multiplicationRight = gateRight[i].value - 1;
    			
    		} else {
    			
    			additionRight = gateRight[i].value;
    			
    		}
    		
    		long _leftLaneValue = leftLaneValue + multiplicationLeft * maxValue;
    		long _rightLaneValue = rightLaneValue + multiplicationRight * maxValue;
    		long _additionalValue = additionalValue + maxValue * (additionLeft + additionRight);
    		
    		leftLaneValue = _leftLaneValue;
    		rightLaneValue = _rightLaneValue;
    		additionalValue = _additionalValue;
    		
    	}
    	
    	sum = leftLaneValue + rightLaneValue + additionalValue;
    ...
    private static void input() throws IOException {
    	...
    	for(int i = 0; i < n; ++i) {
    		...
    		x[i] = in.nextCharacter();
    		L[i] = in.nextLong();
    		
    		y[i] = in.nextCharacter();
    		R[i] = in.nextLong();
