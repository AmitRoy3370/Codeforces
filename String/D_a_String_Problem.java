...
public class D_a_String_Problem {
    	...
    	if(list.size() == 0) {
    		...
    		for(int len = 1; len <= n; ++len) {
    			...
    			for(int j = len; j < m; ++j) {
    				...
    				if(x[list.get(j)] != x[list.get(j - len)]) {
    				...
    				if(j % len != 0 && list.get(j) - list.get(j - len) != list.get(j - 1) - list.get(j - len - 1)) {
    			...
    			for(int j = len; j < m; j += len) {
    				...
    				length = Math.min(length, list.get(j) - list.get(j - 1) - 1);
    				
    			}
    			
    			for(int j = 0; j <= left; ++j) {
    				
    				sum += Math.max(0L, Math.min(length - j + 1, right + 1));
