...
public class E_Eat_the_Chip {
    ...
    static long xAlice, yAlice, xBob, yBob, A_, B_, N_, K, sum = 0L, G, a1, ak, X, Y, A, B, C, l_, t_, P;
    ...
    static void solve(int t) {
    	...
    	long rawWiseDifference = Math.abs(xAlice - xBob);
    	
    	if(rawWiseDifference % 2L == 1L) {
    		
    		long connectingPoint = rawWiseDifference / 2L;
    		
    		if(yAlice > yBob) {
    			
    			yAlice -= connectingPoint;
    			yBob -= connectingPoint;
    		...
    		yAlice = Math.max(1L, yAlice);
    		yAlice = Math.min(yAlice, w);
    		
    		yBob = Math.max(1L, yBob);
    		yBob = Math.min(yBob, w);
    		
    		long coloumnWiseDifference = Math.abs(yAlice - yBob);
    		
    		if(1L >= coloumnWiseDifference) {
    		...
    		yAlice = Math.max(1L, yAlice);
    		yAlice = Math.min(w, yAlice);
    		
    		yBob = Math.max(1L, yBob);
    		yBob = Math.min(w, yBob);
    		
    		long coloumnWiseDifference = Math.abs(yAlice - yBob);
    		
    		if(coloumnWiseDifference == 0L) {
