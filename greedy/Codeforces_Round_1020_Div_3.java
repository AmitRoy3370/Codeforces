...
public class Codeforces_Round_1020_Div_3 {
    ...
    static void C(int t) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (A[1][i] != -1L) {
                ...
                left = Math.max(left, A[0][i] + A[1][i]);
                right = Math.min(right, A[0][i] + A[1][i]);

            } else {

                left = Math.max(left, A[0][i]);
                right = Math.min(right, K + A[0][i]);
