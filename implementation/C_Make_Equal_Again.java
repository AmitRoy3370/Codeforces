...
public class C_Make_Equal_Again {
    ...
    static void solve(int t) {
    	...
    	while(n > first && a[first] == a[0]) {
    		
    		++first;
    		
    	}
    	
    	while(last >= 0 && a[last] == a[n - 1]) {
    		
    		--last;
    		
    	}
    	
    	if(n > first && last >= 0 && a[0] != a[n - 1]) {
    		
    		int makeAllFirst = n - first;
    		int makeAllLast = last + 1;
    		
    		ans.append(Math.min(makeAllFirst, makeAllLast));
    		
    	} else {
    		
    		ans.append(Math.max(0, last - first + 1));
