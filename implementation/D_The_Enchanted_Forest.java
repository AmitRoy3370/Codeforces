...
public class D_The_Enchanted_Forest {
    ...
    static void solve(int t) {
		...
		for(int i = 1; i <= n; ++i) {
			...
			prefix[i] = Long.parseLong( sum(String.valueOf(prefix[i - 1]) , String.valueOf(a[i - 1])));
		...
		if(n > k) {
		    ...
		    for(int i = (int)k; i <= n; ++i) {
			   ...
			   max = Math.max(max, Long.parseLong(sub(prefix[i] + "" , prefix[i - (int)k] + "")));
			
		    }
		
			max = Long.parseLong(sum(max + "", (Long.parseLong(mul(k + "" , (k - 1) + "" ) ) / 2 ) + ""));
			
		} else {
			
			max = Long.parseLong(sum(prefix[n] + "", sum((Long.parseLong(mul(n + "", (n - 1) + "") ) / 2) + "", mul(sub(k + "" , n + "") , n + ""))));
