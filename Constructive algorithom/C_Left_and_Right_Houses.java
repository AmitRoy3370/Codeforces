...
public class C_Left_and_Right_Houses {
    ...
    static void solve(int t) {
    	...
    	for(int i = 1; i != n; ++i) {
    		...
    		prefix[i] = prefix[i - 1] + (x[i] - '0');
    		
    	}
    	
    	int devider = -1;
    	
    	if(prefix[n - 1] >= (int)Math.ceil((double)n / 2.0)) {
    		
    		devider = 0;
    		
    	} else if((n - prefix[n - 1] >= (int)Math.ceil((double)n / 2.0))) {
    	...
    	for(int i = 0; i != n; ++i) {
    		...
    		if(zero >= one) {
    			...
    			one = prefix[n - 1] - prefix[i];
    			zero = ((n - len) - one);
    			
    			if(one >= zero) {
    				
    				if(Math.abs(n - 2 * devider) > Math.abs(n - 2 * len)) {
