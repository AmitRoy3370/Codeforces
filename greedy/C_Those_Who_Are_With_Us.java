...
public class C_Those_Who_Are_With_Us {
    ...
    static long a[], b[], c[], v[], L[], R[], suffix[], prefix[], A[][];
    ...
    static long A_, B_, N_, K, sum = 0L, G, a1, ak, X, Y, B, C, l_, t_, P;
    	...
    	for(int i = 0; i < n; ++i) {
    		...
    		for(int j = 0; j < m; ++j) {
    			...
    			if(A[i][j] == max) {
    				...
    				if(rawCount[i] + colCount[j] == count + 1 ) {
    			...
    			} else if(rawCount[i] + colCount[j] == count) {
