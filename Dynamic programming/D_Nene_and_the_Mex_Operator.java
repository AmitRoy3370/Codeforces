...
public class D_Nene_and_the_Mex_Operator {
    ...
    private static void solve(ArrayList1<String> pairs, int l, int r, int R) {
    ...
    static void solve(int t) {
    	...
    	ArrayList1<String> pairs = new ArrayList1<>();
    	...
    	dp = new long[n + 1];
    	int segmentLen[] = new int[n + 1];
    	
    	for(int i = 1; i <= n; ++i) {
    		
    		for(int j = 1; j <= i; ++j) {
    			
    			sum = dp[i - j] + Math.max(j * j, prefix[i] - prefix[i - j]);
    	...
    	for(int point = n; point > 0; point -= segmentLen[point]) {
    		
    		if(segmentLen[point] == 1) {
    			
    			if(a[point] <= 0L) {
    				
    				pairs.add(point + " " + point);
    		...
    		for(int i = point - segmentLen[point] + 1; i != point + 1; ++i) {
    		...
    		solve(pairs, point - segmentLen[point] + 1, point, point);
    		
    	}
    	
    	m = pairs.size();
    	
    	ans.append(sum).append(" ").append(m);
