...
public class Educational_Codeforces_Round_168_Rated_for_Div_2 {
    ...
    static int timer = 0, testCases, f, n, x_, y_, k, d, w, h, a11, b1, g, m, r, q, a_, b_, c_;

    static char x[], y[], z[][], _A[][], _B[][];

    static long a[], b[], c[], suffix[], prefix[], A[][], B[][];
    ...
    static long N, K, M, sum = 0L, G, a1, ak, X, Y, C, l_, t_, P, R_, N_, X1, Y1;
    ...
    static void bfs(int u, List<Integer> list[]) {
        ...
        for(int vertices : list[u]) {
            
            bfs(vertices, list);
            
            min = Math.min(min, a[vertices]);
        ...
        switch (x[i]) {
            case '_' -> {
                if (list.size() > 0) {
                    ...
                    sum = Long.parseLong(sum(sum + "", sub(i + "", index + "")));
    ...
    static int B(int i, int numberOfType) {
        ...
        int left = p[i - 1];
        int right = p[n - 1] - left - 2;

        if (isSmaller("0", left + "") && isSmaller("0", right + "")) {

            if (x[i] == '.' && y[i - 1] == 'x' && y[i + 1] == 'x' && x[i] == y[i]) {

                numberOfType = Integer.parseInt(sum("1", numberOfType + ""));

            } else if (y[i] == '.' && y[i] == x[i] && x[i - 1] == 'x' && x[i + 1] == 'x') {
        ...
        return B(i + 1, numberOfType);

    }

    static void B(int i, int p[], int count) {
    ...
    static void A(int t) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int j = 0; j < 26; ++j) {
                ...
                var time_ = A(((new String(x).substring(0, i)) + (ch[j] + "") + (new String(x).substring(i))).toCharArray());

                if (isSmaller(timeTakenToType + "", time_ + "") || equal((long) timeTakenToType, (long) time_)) {

                    timeTakenToType = time_;

                    y = ((new String(x).substring(0, i)) + (ch[j] + "") + (new String(x).substring(i))).toCharArray();
