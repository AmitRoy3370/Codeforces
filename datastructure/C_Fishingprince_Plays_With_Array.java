...
public class C_Fishingprince_Plays_With_Array {
    ...
    static void solve(int t) {
		...
		for(long i : a) {
			...
			sumA = Long.parseLong(sum(i + "", sumA + ""));
			
		}
		
		for(long i : b) {
			
			sumB = Long.parseLong(sum(i + "", sumB + ""));
		...
		if(sumA != sumB) {
			...
			ArrayList1<element> A = new ArrayList1<>();
			ArrayList1<element> B = new ArrayList1<>();
			...
			for(int i = 0; i < n; ++i) {
				...
				if(!A.isEmpty() && A.get(0).element == a[i]) {
				...
				A.inserFirst(new element(a[i], count));
			...
			for(int i = 0; i < k; ++i) {
				...
				if(!B.isEmpty() && B.get(0).element == b[i]) {
				...
				B.inserFirst(new element(b[i], count));
			...
			if(A.size() != B.size()) {
				...
				while(!A.isEmpty() && !B.isEmpty()) {
					...
					if(e1.element != e2.element || e1.count != e2.count) {
	...
	static class element {
		...
		public element(long element, long count) {
