...
public class D_The_Omnipotent_Monster_Killer {
    ...
    private static long dfs(long dp[][], int source, int step, int parent) {
    	
    	long damage = a[source] * step;
    	
    	if(dp[source][step] != -1) {
    		
    		return dp[source][step];
    	...
    	for(int i : list[source]) {
    		...
    		for(int j = 1; j <= m; ++j) {
    			...
    			minDamage = Math.min(minDamage, K);
