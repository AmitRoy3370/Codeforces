...
public class G_Mukhammadali_and_the_Smooth_Array {
    ...
    static long G(int i, long savedCost) {
    	...
    	dp[i] = Long.parseLong(sum(partialBest + "", c[i] + ""));
    	
    	savedCost = isSmaller(savedCost + "", dp[i] + "") ? dp[i] : savedCost;
    	
    	return G(i + 1, savedCost);
    	
    }
    
    static long sum(int i, long c[]) {
    	...
    	sum = Long.parseLong(sum(sum + "", c[i] + ""));
    ...
    static void solve(int t) {
    	...
    	savedCost = G(0, savedCost);
    	
    	long cost = Long.parseLong(sub(sum + "", savedCost + ""));
