...
public class Codeforces_Round_835_Div_4 {
	...
	static void F(int t) {
		...
		Given Data: we have total n number of
		...........
		question. the ith question has the a(i)
		number.Can not reapeat the same question
		again for the k day's.We can get maximum
		c coin's over the d day's.
		
		main requirement: we have to determin the
		.................
		maximum value of the k.
		...
		have to repeat value 2 among k = 2 day's.
    ...
    static long arr(long a[]) {

        long result = 0L, count1 = 0L;
    ...
    static void E(int t) {
        ...
        ArrayList1<Integer> one = new ArrayList1<>();
        ArrayList1<Integer> zero = new ArrayList1<>();

        for (int i = 0; i < n; ++i) {

            if (a[i] == 0L && zero.isEmpty()) {
        ...
        if (!one.isEmpty()) {
            ...
            last_one = one.get(one.size() - 1);

        }

        if (!zero.isEmpty()) {

            first_zero = zero.get(zero.size() - 1);

        }

        long final_ans = arr(a);

        if (first_zero > -1) {

            long now = a[first_zero];

            a[first_zero] = 1L;

            final_ans = Math.max(final_ans, arr(a));
    ...
    static void D(int t) {
        ...
        long min = min(a, 0, n, Long.MAX_VALUE);

        int min_index = min_index(a, n, 0, min, 0);
