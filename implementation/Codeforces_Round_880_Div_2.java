...
public class Codeforces_Round_880_Div_2 {
    ...
    static ArrayList1<element> C(long A, long B, long C) {

        ArrayList1<element> list = new ArrayList1<>();

        long minA = pow(10L, A - 1);
        long maxA = pow(10L, A) - 1;
        long minB = pow(10L, B - 1);
        long maxB = pow(10L, B) - 1;

        System.out.println((minB <= maxB) + " " + (minA <= maxA));

        System.out.println(minB + " " + maxB);

        System.out.println(minA + " " + maxA);

        for (long a = Math.min(minA, maxA); a <= Math.max(minA, maxA); ++a) {

            for (long b = Math.min(minB, maxB); b <= Math.max(minB, maxB); ++b) {
                ...
                if ((c + "").length() == C) {
                    ...
                    list.add(new element((long) a, (long) b));
    ...
    static void C(int t) {
        ...
        long min = b[n], max = b[n + 1];

        long sum = 0L;

        boolean ok = false;

        long a = 0L, b_ = 0L, c_ = 0L;

        for (int i = (int) min; i < (int) max; ++i) {

            long mi = b[k], mx = b[k + 1] - 1;

            mi -= i;
            mx -= i;
            mi = Math.max(mi, b[m]);
            mx = Math.min(mx, b[m + 1] - 1);
        ...
        if (ok) {
            ...
            ans.append(a).append(" + ").append(b_).append(" = ").append(c_);
    ...
    static void B(int t) {
        ...
        long ans_ = Long.parseLong(mul(K + "", G + ""));

        long comparator = ans_ - Long.parseLong(mul((((G + 1L) / 2L) - 1L) + "", N + ""));
        ...
        if (comparator < 0L) {
            ...
            ans_ = ans_ - Long.parseLong(mul(((ans_ - (((G + 1) / 2) - 1) * N + G - 1) / G) + "", G + ""));
