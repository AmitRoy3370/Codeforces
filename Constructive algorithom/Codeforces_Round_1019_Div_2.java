...
public class Codeforces_Round_1019_Div_2 {
    ...
    static int timer = 0, testCases, f, n, x_, y_, k, w, h, a11, b1, g, m, m1, m2, r, q, a_, b_, c_, start, end, l;
    ...
    static void D(int t) {
        ...
        int notRemovedElement = -1;

        for (int i = 0; i < n; ++i) {

            if (a[i] == -1) {

                notRemovedElement = i + 1;
        ...
        for (int operationNumber = 1; operationNumber <= 500; ++operationNumber) {

            for (int i = 1; i < notRemovedElement; ++i) {

                if (a[i - 1] == operationNumber) {

                    if (operationNumber % 2 == 1) {
            ...
            for (int i = n; i > notRemovedElement; --i) {
    ...
    static void B(int t) {
        ...
        if (equal(notAdjacent * 1L, zeros * 1L) && equal(notAdjacent * 1L, 2L)) {

            zeros = 1;

        }

        sum = Long.parseLong(sum(n + "", notAdjacent + ""));
        sum = Long.parseLong(sub(sum + "", zeros + ""));
