...
public class F_Chat_Screenshots {
    ...
    static ArrayList1<ArrayList1<Long>> list_ = new ArrayList1<>();

    static int[] dx = {1, -1, 0, 0, 1, -1, 1, -1};
    static int[] dy = {0, 0, 1, -1, -1, 1, 1, -1};
    static int dir[][] = new int[][]{{0, 1}, {1, 0}, {-1, 0}, {0, -1}};
    
    static boolean cycle(boolean visited[], boolean dfsVisited[], int node) {
    	
    	visited[node] = true;
    	dfsVisited[node] = true;
    	
    	for(int vertex : list[node]) {
    		
    		if(!visited[vertex]) {
    			
    			if(cycle(visited, dfsVisited, vertex)) {
