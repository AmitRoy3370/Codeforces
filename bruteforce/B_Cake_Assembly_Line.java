...
public class B_Cake_Assembly_Line {
	...
	static void solve() {
		...
		for(int i = 0; i < n; ++i) {
			...
			A[i] = new diff(a[i] - w, a[i] + w);
			B[i] = new diff(b[i] - h, b[i] + h);
			
		}
		
		long left_shift = Long.MAX_VALUE;
		
		for(int i = 0; i < n; ++i) {
			
			long shift = B[i].lower_bound - A[i].lower_bound;
			
			left_shift = Math.min(left_shift, shift);
		...
		for(int i = 0; i < n; ++i) {
			...
			if(A[i].higher_bound >= B[i].higher_bound) {
				
				continue;
				
			}
			
			if((B[i].higher_bound - A[i].higher_bound) <= left_shift) {
	...
	static class diff {
		...
		long lower_bound, higher_bound;
		
		public diff(long lower_bound, long higher_bound) {
