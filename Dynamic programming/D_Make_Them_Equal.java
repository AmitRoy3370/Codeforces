		...
		for(int i = 1; i <= 1000; ++i) {
			...
			for(int x = 1; x <= i; ++x) {
				...
				try {
					...
					dp[i + i / x] = Math.min(dp[i] + 1, dp[i + i / x]);
		...
		for(int i = 0; i < n; ++i) {
			...
			for(int j = k; j >= b[i]; --j) {
				...
				if(j >= b[i]) {
					...
					profit[j] = Math.max(profit[j], profit[j - b[i]] + c[i]);
