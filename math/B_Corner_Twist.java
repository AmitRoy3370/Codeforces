...
public class B_Corner_Twist {
    ...
    private static void change(int i, int j, int type) {
    	
    	if(type == 1) {
    		
    		_A[i][j] = ((((_A[i][j] - '0') + 1) % 3) + "").charAt(0);
    		
    	} else {
    		
    		_A[i][j] = ((((_A[i][j] - '0') + 2) % 3) + "").charAt(0);
    ...
    private static boolean equal(int i, int j) {
    	...
    	return ((((_A[i][j] - '0') + 1) % 3) + "").charAt(0) == _B[i][j];
    ...
    private static void input() throws IOException {
    	...
    	for(int i = 0; i < n; ++i) {
    		...
    		_A[i] = in.next().toCharArray();
    		
    	}
    	
    	for(int i = 0; i < n; ++i) {
    		
    		_B[i] = in.next().toCharArray();
