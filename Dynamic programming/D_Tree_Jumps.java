...
public class D_Tree_Jumps {
    ...
    static void solve(int t) {
    	...
    	TreeMap<Long, Integer> levelCount = new TreeMap<>();
    	
    	long maxDepth = 0;
    	
    	for(long i : depth) {
    		
    		levelCount.put(i, levelCount.getOrDefault(i, 0) + 1);
    		
    		maxDepth = Math.max(maxDepth, i);
    		
    	} 
    	
    	dp = new long[(int)maxDepth + 2];
    	
    	dp[(int)maxDepth] = levelCount.get(maxDepth);
    	
    	for(long level = maxDepth - 1; level >= 1; --level) {
    		
    		long wayToStopAtPresentLabel = levelCount.getOrDefault(level, 0) % mod;
    		
    		long wayToGoAtNextLabel = (levelCount.getOrDefault(level, 0) - 1 + mod) % mod;
    		
    		wayToGoAtNextLabel = wayToGoAtNextLabel * dp[(int)level + 1];
    		
    		wayToGoAtNextLabel %= mod;
    		
    		dp[(int)level] = (wayToStopAtPresentLabel + wayToGoAtNextLabel) % mod;
