// Problem: E. Common Generator
// Contest: Codeforces - Refact.ai Match 1 (Codeforces Round 985)
...
public class E_Common_Generator {
    ...
    static void generateDp() {
    	
    	dp = new long[(int)(4e5 + 5)];
    	
    	for(long i = 2; i <= (int)(4e5); ++i) {
    		
    		if(dp[(int)i] == 0) {
    			
    			for(long j = i * i; j <= (int)(4e5); j += i) {
    				
    				if(j < dp.length && dp[(int)j] == 0) {
    ...
    static void solve(int t) {
    	...
    	if(generatorRoot == -1) {
    		...
    		for(long i : a) {
    			...
    			if(i % generatorRoot != 0 && 2L * generatorRoot > i - dp[(int)i]) {
