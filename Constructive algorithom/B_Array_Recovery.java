...
public class B_Array_Recovery {
    ...
    static int testCases, c, n;

    static int a[];

    static ArrayList1<Long> stack[];

    static void solve(int i, int new_array[], int t) {
        ...
        int value1 = Integer.parseInt(sum(a[i] + "", new_array[i - 1] + ""));
        int value2 = (new_array[i - 1] - a[i]);

        if ((value1 >= 0 && value2 >= 0 && pow(value1, 2L) != pow(value2, 2L))) {
        ...
        new_array[i] = Math.max(value1, value2);

        solve(i + 1, new_array, t);
