...
public class D_Retaliation {
    ...
    static long A_, B_, N_, K, sum = 0L, sub, G, a1, ak, X, Y, A, B, C, l_, t_, P;
    ...
    static void solve(int t) {
    	...
    	if(isSmaller(a[1] + "", a[0] + "")) {
    		
    		reverse(a);
    		
    	}
    	
    	long difference = Long.parseLong(sub(a[1] + "", a[0] + ""));

        boolean yes = true;
        
        for(int i = 0; i < n - 1; ++i) {
        	
        	long _difference = a[i + 1] - a[i];
        	
        	if(!equal(_difference, difference)) {
        ...
        if(!yes) {
        	...
        	if(equal(a[0], difference) || isSmaller(difference + "", a[0] + "")) {
        		
        		sub = Long.parseLong(sub(a[0] + "", difference + ""));
