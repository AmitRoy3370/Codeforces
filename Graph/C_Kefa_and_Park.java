...
public class C_Kefa_and_Park {
    ...
    static void dfs(int src, long H, int h[][], boolean visited[], List<Integer> list[]) {
    	
    	//k = (p[src] == 1) ? k + 1 : 0;
    	//h[src][1] = k;
    	H = Math.max(H, h[src][1]);
    	...
    	for(int i : list[src]) {
    		...
    		if(!visited[i]) {
    			...
    			visited[i] = true;
    			h[i][1] = (p[i] == 1 ? h[src][1] + 1 : 0);
    			
    			dfs(i, H, h, visited, list);
    			h[src][0]++;
