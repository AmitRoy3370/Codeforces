...
public class B_Bus_of_Characters{
    ...
    static void solve(){
		...
		for(int i=0;i<n;i++){
			...
			elements[i]=new element(width[i],i+1 );
			
		}
		
		Arrays.sort(elements);
		
		Stack<element> open=new Stack<>();
		
		Stack<element> close=new Stack<>();
		
		for(int i=0;i<n;i++){
			
			open.push(elements[i] );
	...
	static class element implements Comparable<element> {
		...
		public element(long width,int index){
		...
		public int compareTo(element t){
			...
			//return Long.compareTo(this.width,t.width );
