...
public class Codeforces_Round_982_Div_2 {
        ...
        for (int i = m - 1; i >= 0; --i) {
            ...
            for (int j = n - 1; j >= 0; --j) {
                ...
                if (k >= j) {
                    ...
                    dp[j] = Math.min(dp[j], m - i - 1 + dp[k + 1]);
    ...
    static long C(long source, TreeMap<Long, ArrayList1<Long>> map, TreeMap<Long, Long> dp) {
        ...
        if (map.containsKey(source)) {
            ...
            while (!map.get(source).isEmpty() && map.get(source).size() > 0) {

                long value = (((source + map.get(source).get(0) - 1) > 0) ? (Long.parseLong(sum(source + "", (map.get(source).get(0) - 1) + ""))) : (source + map.get(source).get(0) - 1));

                max = Math.max(max, C(value, map, dp));

                map.get(source).popFront();
    ...
    static void C(int i, TreeMap<Long, ArrayList1<Long>> map) {
        ...
        if (!map.containsKey(value)) {
            ...
            map.put(value, new ArrayList1<>());

        }

        map.get(value).add((long) i - 1);
    ...
    static void C(int t) {
        ...
        TreeMap<Long, ArrayList1<Long>> map = new TreeMap<>();
        ...
        while (!pq.isEmpty()) {
            ...
            if (map.containsKey(value)) {
                ...
                while (!map.get(value).isEmpty()) {
                    ...
                    pq.add(map.get(value).get(0) + value);

                    list.add(map.get(value).get(0));

                    map.get(value).popFront();

                }

                while (!list.isEmpty()) {

                    map.get(value).add(list.get(0));
    ...
    static int B(int i, int j, int k) {
        ...
        if (equal((long) j, (long) n) || isSmaller(n + "", j + "")) {
    ...
    static void B(int i, int k) {

        if (equal((long) i, (long) n) || isSmaller(n + "", i + "")) {
        ...
        long value = Long.parseLong(sum(B(i, i + 1, k) + "", i + ""));

        if (isSmaller(value + "", sum + "")) {
    ...
    static void A(int t) {
        ...
        long maxA = max(a, n, 0, a[0]);
        long maxB = max(b, n, 0, b[0]);

        sum = Long.parseLong(sum(mul(2 + "", maxA + ""), mul(2 + "", maxB + "")));
