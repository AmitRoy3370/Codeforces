...
public class D_Balanced_Tree {
    ...
    static long a[], b[], c[], L[], R[], suffix[], prefix[];
    ...
    static void dfs(int source, int parent, List<Integer> list[]) {
    	
    	dp[source] = L[source]; // set the min most value as the value of a(i)
    	
    	for(int neighbour : list[source]) { // traverse all the neighbour of the source
    		
    		if(neighbour == parent) { // for repeated value
    			
    			continue;
    			
    		}
    		
    		dfs(neighbour, source, list); // go at the next depth
    		
    		if(dp[neighbour] > dp[source]) { // our source value will max always, so when this condition is violated then need to some change at here
    			
    			if(dp[neighbour] > R[source]) {
    				
    				sum += dp[neighbour] - R[source]; // collected the nessesary added value from the operation
    				dp[source] = R[source]; // change the source value to it's highest range, so that next operation can be performed easily at here
    				
    			} else {
    				
    				dp[source] = dp[neighbour]; // set the max value of the neighbour as the value of the source if it can be set at there
    ...
    static void solve(int t) {
    	...
    	dp = new long[n]; // initial value of a
    	
    	sum = 0; // have to increase via operation
    	
    	dfs(0, -1, list);
    	
    	long balancedValue = dp[0] + sum; // final balance value = max(a) + increased value
    	
    	ans.append(balancedValue);
