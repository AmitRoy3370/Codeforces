...
public class A_El_fucho {
    ...
    static void solve(int t) {
    	...
    	int winner = n, looser = 0;
    	
    	while(winner > 1 || looser > 1) {
    		
    		sum = Long.parseLong(sum(sum + "", (winner / 2) + ""));
    		sum = Long.parseLong(sum(sum + "", (looser / 2) + ""));
    		
    		looser = Integer.parseInt(sub(looser + "", (looser / 2) + ""));
    		looser = Integer.parseInt(sum(looser + "", (winner / 2) + ""));
    		
    		winner = Integer.parseInt(sub(winner + "", (winner / 2) + ""));
    		
    	}
    	
    	if(equal(winner * 1L, 1L) && equal(looser * 1L, 1L)) {
