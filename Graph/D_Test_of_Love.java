...
public class D_Test_of_Love {
    ...
    static record Swimmer(int pos, int swim) implements Comparable<Swimmer> {
    ...
    static void solve(int t) {
    	...
    	Queue<Swimmer> queue = new PriorityQueue<>();
    	
    	queue.add(new Swimmer(0, 0));
    	...
    	while(!queue.isEmpty()) {
    		...
    		if(pos == 0 || x[pos - 1] == 'L') {
    			...
    			for(int j = 1; j <= m; ++j) {
    				...
    				if(newPos <= n && x[newPos - 1] != 'C' && !visited[newPos]) {
    					
    					visited[newPos] = true;
    					queue.add(new Swimmer(newPos, swim));
    		...
    		if(pos > 0 && pos <= n && x[pos - 1] == 'W' && swim < k) {
    			...
    			if(newPos <= n && x[pos - 1] != 'C' && !visited[newPos]) {
    				
    				visited[newPos] = true;
    				queue.add(new Swimmer(newPos, swim + 1));
