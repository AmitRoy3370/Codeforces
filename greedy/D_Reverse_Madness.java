...
public class D_Reverse_Madness {
    ...
    static int testCases, n, x_, y_, k, count = 1, d, w, h, a11, b1, f, g, m, s, r, q, a_, b_, c_, N = (int) (1e6 + 1), L[], R[];

    static char x[], y[], z[];

    static long a[], b[], c[], v[], suffix[], prefix[];

    static long dp[];

    static int p[][], trie[], indexes[];
    ...
    static void solve(int t) {
    	...
    	int prefix[] = new int[n + 20];
    	
    	for(int i = 0; i != q; ++i) {
    		
    		int index = upper_bound(copyL, k - 1, indexes[i]) - L[0];
    		
    		int left = Math.min(indexes[i], L[index] + R[index] - indexes[i]);
    		int right = Math.max(indexes[i], L[index] + R[index] - indexes[i]);
    		
    		/*if(new String(x).equals("abcd")) {
    			
    			System.out.println(right + " " + left + " " + index + " " + (i + 1) + " " + L[index] + " " + R[index] + " " + indexes[i]);
    ...
    static int lower_bound(int a[], int value) {
