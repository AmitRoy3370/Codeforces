...
public class C_Nikita_and_LCM {
    ...
    static void solve(int t) {
    	...
    	for(int i = n - 2; i >= 0; --i) {
    		...
    		for(long value : dp.keySet()) {
    			
    			long lcm = lcm(a[i], value);
    			...
    			map.put(lcm, Math.max(map.getOrDefault(lcm, 0), dp.getOrDefault(value, 0) + 1));
    			
    		}
    		
    		for(long value : map.keySet()) {
    			
    			dp.put(value, Math.max(map.get(value), dp.getOrDefault(value, 0)));
    			
    		}
    		
    		dp.put(a[i], Math.max(dp.getOrDefault(a[i], 0), 1));
    	...
    	for(long i : dp.keySet()) {
    		...
    		length = Math.max(length, dp.get(i));
